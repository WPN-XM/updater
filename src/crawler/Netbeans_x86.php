 ...
 */
class Netbeans_x86 extends VersionCrawler
{
    public $name = 'netbeans-x86';
    ...
    public function crawlVersion()
    {
        return $this->filter('head > script')->each(function ($node) {      
            ...
            if (preg_match("#PAGE_ARTIFACTS_LOCATION = \"(.*)\";#", $node->text(), $matches)) {
                ...
                if (version_compare($version, $this->registry['netbeans-x86']['latest']['version'], '>=') === true) {
                    return array(
                        'version' => $version,  
                        // http://download.netbeans.org/netbeans/8.2/final/bundles/netbeans-8.2-php-windows-x86.exe                         
                        'url'     => 'http://download.netbeans.org/netbeans/' . $version . '/final/bundles/netbeans-' . $version . '-php-windows-x86.exe',
                    );
                }
            }
        });
